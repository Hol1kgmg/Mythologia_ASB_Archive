 ...
 */
export function requireAdminRole(requiredRoles: string[]): MiddlewareHandler<{ Variables: AdminAuthContext }> {
  return async (c, next) => {
    ...
    if (!requiredRoles.includes(admin.role)) {
      logger.warn(`Access denied for admin ${admin.username}: required roles ${requiredRoles.join(', ')}, has ${admin.role}`);
      return c.json({
        error: 'Forbidden',
 ...
 */
export function requireAdminPermission(requiredPermissions: string[]): MiddlewareHandler<{ Variables: AdminAuthContext }> {
  return async (c, next) => {
    const admin = c.get('admin');
    ...
    if (!hasPermission) {
      logger.warn(`Access denied for admin ${admin.username}: required permissions ${requiredPermissions.join(', ')}, has ${admin.permissions.join(', ')}`);
      return c.json({
        error: 'Forbidden',
