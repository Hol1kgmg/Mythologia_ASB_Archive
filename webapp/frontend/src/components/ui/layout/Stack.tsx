...
const stackVariants = cva('flex', {
  variants: {
    ...
    divider: {
      ...
      vertical:
        '[&>*:not(:last-child)]:border-b [&>*:not(:last-child)]:border-gray-700 [&>*:not(:last-child)]:pb-4',
      false: '',
    },
  },
  defaultVariants: {
...
const Stack = forwardRef<HTMLDivElement, StackProps>(
  (
    { className, direction, spacing, align, justify, wrap, divider, children, as, ...props },
    ref
  ) => {
...
const Spacer = forwardRef<HTMLDivElement, SpacerProps>(
  ({ className, size = 'auto', ...props }, ref) => {
    ...
    return <Box ref={ref} className={`${spacerClass} ${className || ''}`} {...props} />;
  }
);
