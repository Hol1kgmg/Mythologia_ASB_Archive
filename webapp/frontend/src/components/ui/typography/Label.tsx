...
const labelVariants = cva('font-medium inline-block transition-colors', {
  variants: {
    size: {
      xs: 'text-xs',
      sm: 'text-sm',
...
export const BadgeLabel = React.forwardRef<HTMLLabelElement, BadgeLabelProps>(
  ({ className, variant, rounded, children, ...props }, ref) => {
    return (
      <label ref={ref} className={badgeVariants({ variant, rounded, className })} {...props}>
        {children}
      </label>
    );
  }
...
export const IconLabel = React.forwardRef<HTMLLabelElement, IconLabelProps>(
  ({ className, icon, iconPosition = 'left', spacing = 'sm', children, ...props }, ref) => {
    const spacingClass = {
      xs: 'gap-1',
      sm: 'gap-2',
      md: 'gap-3',
